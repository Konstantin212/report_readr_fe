...
export function TaxClient({ year: yearNum }: { year: number }) {
    ...
    return (
      <main className="space-y-4">
        <h1 className="text-2xl font-bold tracking-tight">
          Tax <span className="font-mono text-sm text-muted ml-1 tracking-wider">{yearNum} · Germany</span>
        </h1>
        <Card>
          <div className="flex items-center gap-3 text-muted text-sm">
            <span className="inline-block w-4 h-4 rounded-full border-2 border-amber/40 border-t-amber animate-spin" />
            Loading tax data…
          </div>
        </Card>
      </main>
      ...
      <main className="space-y-4">
        ...
        <Card>
          <div className="text-bad text-sm flex items-center justify-between gap-3">
            <span>Couldn’t load tax data.</span>
            <button type="button" onClick={() => q.refetch()} className="font-mono text-[11px] uppercase tracking-widest border border-borderHard px-3 py-1.5 rounded-md">Retry</button>
          </div>
        </Card>
      </main>
    );
  ...
  const { tax: d, availableYears } = q.data;
  const year = yearNum;
